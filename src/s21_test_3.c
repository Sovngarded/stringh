...
int unsigned_decimal_to_string(char *str_buff, Options options,
                               unsigned long int buff, s21_size_t size_num);
...
int write_to_string(long int number, Options options, char *string_for_number,
                    s21_size_t size);
...
s21_size_t get_size(long int number, Options *options) {
  ...
  if (result == 0 && number == 0 && !options->width && !options->accuracy &&
...
int write_to_string(long int number, Options options, char *string_for_number,
  ...
  if ((number == 0 &&
      ...
      (number == 0 && !options.accuracy && !options.width &&
       !options.is_blank && !options.is_dot)) {
  ...
  while (options.is_zero && string_for_number &&
         (size - options.flag_size > 0) && (options.accuracy || change_sign)) {
...
//     if(format == 'l') buff = (unsigned long int)va_arg(*arg,unsigned long
//     int); else if(format == 'h') buff = (unsigned short
//     int)va_arg(*arg,unsigned short int); else buff = (unsigned
//     int)va_arg(*arg,unsigned int);
...
// int unsigned_decimal_to_string(char *str_buff,Options options, unsigned long
// int buff, s21_size_t size_num){
//     int i = 0, flag =0;
//     unsigned long int copy_buff = buff;

//     // **********************************************************************
//     // if((options.is_hash && options.number_system == 8)) options.flag_size
//     = 1; else if (options.is_hash && options.number_system == 16)
//     options.flag_size = 2;
...
//     )
//     {
//        char sym = get_num_char(copy_buff % options.number_system,
//        options.upper_case); size_num[i] = sym;
//         i++;
//         size_num--;
//         copy_buff /= 10;
//     }
...
//     if(size_decimal == 1 && options.is_zero == 1 && options.flag_size = 1)
//     options.is_zero = 0;
